...
int send_ota_start(int comport)
{
  ...
  if( ex >= 0 )
  {
    printf("Sent SOF, waiting for ack\n");
...
uint16_t send_ota_end(int comport)
{
  ...
  // len = sizeof(ETX_OTA_EOF);

  //send OTA END
  for(int i = 0; i < 2; i++)
  {
    delay(1);
    if( RS232_SendByte(comport, ETX_OTA_EOF) )
    {
      //some data missed.
...
int send_ota_data(int comport, uint8_t *data, uint16_t data_len, uint16_t app_size)
{
  uint16_t len;
  ...
  ota_data->sof          = ETX_OTA_SOF;                 //1B
  ota_data->packet_type  = ETX_OTA_PACKET_TYPE_DATA;    //1B
  ota_data->data_len     = data_len;                    //2B
  ota_data->fw_size      = app_size;                    //2B
  ...
  len++;
  //DEBUG
  //printf("OTATOOL: Total PACKET LENGTH of chunk is %d \n", len);
  ...
  if( ex >= 0 )
  {
    //printf("DATA PACKET is sent, waiting for acknowledgement\n");
    if( !is_ack_resp_received( comport ) )
    {
      //Received NACK
...
int main(int argc, char *argv[])
{
  ...
  do
  {
    ...
    for( uint32_t i = 0; i < app_size; )
    {
      ...
      {
        size = app_size - i;
        printf("last app chunk\n");
      }

      printf("[%d/%d], FW CHUNK SIZE : %d\r\n", i/ETX_OTA_DATA_MAX_SIZE, app_size/ETX_OTA_DATA_MAX_SIZE,size);

      ex = send_ota_data( comport, &APP_BIN[i], size,app_size );
      if( ex < 0 )
      {
